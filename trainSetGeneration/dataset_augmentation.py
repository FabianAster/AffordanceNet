...
def apply_transformations(counter, dir_img_src, dir_img_dest, dir_xml_src, dir_xml_dest, dir_mask_src, dir_mask_dest, angle, img_width, img_height):
...
def mirror_masks(src_dir, dest_dir, counter, filename, num_objects):
    ...
    for i in range(1, num_objects + 1):
        ...
        mirror_image(mask_path_src, mask_path_dest)

# Function to rotate the masks
def rotate_masks(src_dir, dest_dir, counter, filename, angle, num_objects):
    base_name = os.path.splitext(filename)[0]
    for i in range(1, num_objects + 1):
        mask_filename = f"{base_name}_{i}_segmask.png"
        mask_filename_dest = f"{counter}_{i}_segmask.png"
        ...
        rotate_image(mask_src_dir, mask_dest_dir, angle)
...
for angle in range(0, 360, 10):
    ...
    counter = apply_transformations(counter, imgs_src, imgs_dest, xml_src, xml_dest, mask_src, mask_dest, angle, image_height, image_width)
    counter+=1